...
export const GAME_PLAY_MODE_MAP: Record<GamePlayMode, GamePlayModeConfig> =
  Object.fromEntries(GAME_PLAY_MODES.map((m) => [m.value, m])) as Record<GamePlayMode, GamePlayModeConfig>
 ...
 * - autres → tous
 */
export function getAvailableRolesForCriterion(
  criterion: string,
  allRoles: MemberRole[],
): MemberRole[] {
  ...
  if (criterion === 'performance') {
    return allRoles.filter(
      (r) => !(['leader', 'visual', 'maknae'] as MemberRole[]).includes(r),
    )
  }
  return allRoles
}
