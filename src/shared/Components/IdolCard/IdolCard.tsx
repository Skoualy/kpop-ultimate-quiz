...
export function IdolCard({ idol, size, disabled, hideInfos, onClick }: IdolCardProps) {
  const portrait = idol.portrait ?? (idol.gender == 'f' ? PLACEHOLDER_F : PLACEHOLDER_M)

  const handleClick = () => {
    if (!disabled) onClick(idol.idolId)
  }
    ...
    >
      ...
      {!hideInfos && (
        <div className={styles.info}>
          ...
          {idol.isFormer && <span className={styles.formerBadge}>Ancien membre</span>}
        </div>
      )}
    </button>
  )
