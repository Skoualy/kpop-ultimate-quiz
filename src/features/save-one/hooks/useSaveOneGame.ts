...
export interface UseSaveOneGameReturn extends GameState {
  isLoading:            boolean
  error:                string | null
  rounds:               RoundData[]
  results:              RoundResult[]
  choose:               (chosenId: string, timeMs: number) => void
...
export function useSaveOneGame(config: GameConfig): UseSaveOneGameReturn {
    ...
    try {
      ...
      if (config.category === 'idols') {
        ...
        const pool = buildSongPool(groups, {
          ...
          clipDuration: config.clipDuration,
          // PATCH : filtre langue désormais appliqué au pool
  ...
  const recordResult = useCallback((
    ...
    if (isRoundComplete) handleSongMemoryUpdate(currentRoundIndex)

    setResults((r) => [...r, { roundIndex: currentRoundIndex, playerIndex: currentPlayer, chosenId, isPass, isTimeout, timeMs }])

    setState((prev) => {
      const { currentRoundIndex: ri, currentPlayer: cp } = prev
      const totalRounds = roundsRef.current.length
      if (config.twoPlayerMode && cp === 0) return { ...prev, phase: 'playerTransition', timerKey: prev.timerKey + 1 }
      const nextIndex = ri + 1
      if (nextIndex >= totalRounds) return { ...prev, phase: 'summary' }
      return { ...prev, phase: 'roundTransition', currentRoundIndex: nextIndex, currentPlayer: 0 as PlayerIndex, timerKey: prev.timerKey + 1 }
    })
  }, [config.twoPlayerMode, handleSongMemoryUpdate])

  const choose  = useCallback((id: string, ms: number) => recordResult(id,   false, false, ms),   [recordResult])
  const pass    = useCallback((ms?: number)             => recordResult(null, true,  false, ms ?? null), [recordResult])
  const timeout = useCallback(()                        => recordResult(null, true,  true,  null), [recordResult])

  const skipPlayerTransition = useCallback(() =>
    setState((p) => ({ ...p, phase: 'playing', currentPlayer: 1 as PlayerIndex, timerKey: p.timerKey + 1 })), [])
  const skipRoundTransition  = useCallback(() =>
    setState((p) => ({ ...p, phase: 'playing', currentPlayer: 0 as PlayerIndex, timerKey: p.timerKey + 1 })), [])
