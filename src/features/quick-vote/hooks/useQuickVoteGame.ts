...
export function useQuickVoteGame(config: GameConfig): UseQuickVoteGameReturn {
  ...
  const vote    = useCallback((v: 'positive' | 'negative', ms: number) => recordVote(v, false, ms), [recordVote])
  ...
  return { ...state, isLoading, error, rounds, results, vote, timeout, skipPlayerTransition, skipRoundTransition, restart }
}
