...
import type { QuickVote, QuickVoteResult } from '../QuickVotePage.types'
...
export interface UseQuickVoteGameReturn extends GameState {
  ...
  vote: (v: QuickVote, timeMs: number) => void
...
export function useQuickVoteGame(config: GameConfig): UseQuickVoteGameReturn {
  ...
  const recordVote = useCallback(
    (vote: QuickVote, isPass: boolean, isTimeout: boolean, timeMs: number | null) => {
      ...
      setResults((r) => [
        ...r,
        { roundIndex: currentRoundIndex, playerIndex: currentPlayer, isPass, vote, isTimeout, timeMs },
      ])
  ...
  const vote = useCallback((v: QuickVote, ms: number) => recordVote(v, false, false, ms), [recordVote])
  const timeout = useCallback(() => recordVote('negative', false, true, null), [recordVote])
  const pass = useCallback((ms?: number) => recordVote(null, true, false, ms ?? null), [recordVote])
