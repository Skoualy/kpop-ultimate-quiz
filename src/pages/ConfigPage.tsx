...
import type {
  Group, QuizMode, QuizCategory, SaveOneCriterion, MemberRole, SongType, Generation,
} from '../types';
...
export function ConfigPage() {
  ...
  const [filters, setFilters] = useState<FilterState>({
    search: '', gen: 'all', categoryFilter: 'all',
  });
  ...
  const moveRight = () => {
    ...
    setConfig({ selectedGroupIds: [...config.selectedGroupIds, ...ids.filter((id) => !config.selectedGroupIds.includes(id))] });
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div className="page-header__actions">
          <button className="btn btn--ghost btn--sm" onClick={resetConfig}>Reset</button>
        </div>
      </div>
      ...
      <div className="config-grid">
        ...
        {isSaveOne && (
          <div className="config-card">
            ...
            <input type="number" className="input" min={1} max={4}
              value={config.drops}
        ...
        {isSaveOne && isIdols && (
          <div className="config-card">
            ...
            <select className="select" value={config.criterion}
        ...
        {config.category === 'songs' && (
          <div className="config-card">
            ...
            <select className="select" value={config.songType}
        ...
        {isSaveOne && isIdols && (
          <div className="config-card">
            ...
            <select className="select" value={config.roleFilter}
        ...
        {isBlindTest && (
          <div className="config-card">
            ...
            <select className="select" value={config.clipDuration}
        ...
        <div className="config-card">
          ...
          <label className="checkbox-row">
            <input type="checkbox" checked={config.twoPlayerMode}
        ...
        {config.twoPlayerMode && (
          <div className="config-card" style={{ gridColumn: 'span 2' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <input className="input" value={config.player1Name}
                onChange={(e) => setConfig({ player1Name: e.target.value })}
                placeholder="Joueur 1" />
              <input className="input" value={config.player2Name}
      ...
      <div className="group-selector">
        <div className="group-selector__header">
          ...
          <span className="group-selector__count">— {config.selectedGroupIds.length} sélectionné{config.selectedGroupIds.length > 1 ? 's' : ''}</span>
        </div>
        ...
        <div className="group-selector__dual">
            ...
            <div className="group-list">
              {filteredAvailable.map((g) => (
                <div
                  ...
                  style={{ background: selectedAvail.has(g.id) ? 'rgba(124,58,237,0.12)' : undefined }}
                  onClick={() => {
                ...
                >
                  ...
                  <span className="group-list__item-meta">{genIcon(g)} Gen{g.generation}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="group-selector__controls">
            <button className="group-selector__ctrl-btn" title="Tout ajouter" onClick={moveAllRight}>»</button>
            <button className="group-selector__ctrl-btn" title="Ajouter" onClick={moveRight}>›</button>
            <button className="group-selector__ctrl-btn" title="Retirer" onClick={moveLeft}>‹</button>
            <button className="group-selector__ctrl-btn" title="Tout retirer" onClick={moveAllLeft}>«</button>
          </div>
            ...
            <div className="group-list">
              {selectedGroups.map((g) => (
                <div
                  ...
                  style={{ background: selectedChosen.has(g.id) ? 'rgba(124,58,237,0.12)' : undefined }}
                  onClick={() => {
                  ...
                  onDoubleClick={() => {
                    setConfig({ selectedGroupIds: config.selectedGroupIds.filter((id) => id !== g.id) });
                    setSelectedChosen(new Set());
                  }}
                >
                  <span className="group-list__item-name">{g.name}</span>
      ...
      <div className="config-launch">
        ...
        {!canLaunch && (
          <span style={{ marginLeft: 14, fontSize: 13, color: 'var(--text-muted)' }}>
            Sélectionne au moins un groupe
          </span>
        )}
      </div>
    </div>
