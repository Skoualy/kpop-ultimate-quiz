...
import type {
  Group, GroupCategory, GroupStatus, Generation, NationalityCode,
  MemberRole, MemberStatus, LanguageCode, EditableMemberRow, ContributionBundle,
  Idol, SongEntry,
} from '../types';
import {
  slugify, generateIdolId, getYouTubeThumbnail, getCategoryGender,
} from '../services/dataService';
import { useToast } from '../components/shared';

// ─── Constants ─────────────────────────────────────────────────────────────────
const ALL_ROLES: MemberRole[] = [
  'leader', 'mainVocal', 'vocal', 'mainDancer', 'dancer', 'mainRapper', 'rapper', 'visual', 'maknae',
];
const ROLE_LABELS: Record<MemberRole, string> = {
  leader: 'Leader', mainVocal: 'Main Vocal', vocal: 'Vocal',
  mainDancer: 'Main Dancer', dancer: 'Dancer',
  mainRapper: 'Main Rapper', rapper: 'Rapper', visual: 'Visual', maknae: 'Maknae',
};
const NATIONALITIES: { code: NationalityCode; label: string }[] = [
  { code: 'kr', label: '🇰🇷 Corée' }, { code: 'jp', label: '🇯🇵 Japon' },
  { code: 'cn', label: '🇨🇳 Chine' }, { code: 'tw', label: '🇹🇼 Taïwan' },
  { code: 'th', label: '🇹🇭 Thaïlande' }, { code: 'us', label: '🇺🇸 USA' },
...
function emptyForm(): GroupForm {
  return {
    id: '', name: '', category: 'girlGroup', parentGroupId: '',
    generation: '4', debutYear: String(new Date().getFullYear()),
    status: 'active', company: '', fandomName: '', fandomUrl: '', notes: '',
  };
}
...
function emptySong(): EditableSong {
  return {
    ...
    id: '', title: '', youtubeUrl: '', language: '', isDebutSong: false,
  };
}
...
export function ContributorPage() {
  ...
  function handleDownloadJson() {
    ...
    a.href = url; a.download = `${form.id || 'contribution'}.bundle.json`;
    ...
    <div className="page-container">
      ...
      {step < 3 && (
        <div className="step-nav">
          {step > 0 && (
            <button className="btn btn--ghost" onClick={() => setStep(step - 1)}>← Retour</button>
          )}
          <div style={{ flex: 1 }} />
...
// ─── Step 1: Group Info ─────────────────────────────────────────────────────────
function GroupInfoStep({ form, setForm, topLevelGroups, isEdit }: {
  form: GroupForm;
  ...
  return (
    <div className="contributor-step">
      <div className="card">
        <div className="card__title" style={{ fontSize: 18, marginBottom: 16 }}>Informations générales</div>
        <div className="form-grid-2">
          <div className="form-field">
            <label className="form-label">Nom du groupe <span>*</span></label>
          ...
          <div className="form-field">
            <label className="form-label">Génération <span>*</span></label>
            <select className="select" value={form.generation} onChange={(e) => upd('generation', e.target.value)}>
              {(['1','2','3','4','5'] as Generation[]).map((g) => (
                <option key={g} value={g}>{g}ème génération</option>
              ))}
            </select>
          </div>
          <div className="form-field">
            <label className="form-label">Label / Agence <span>*</span></label>
            <input className="input" value={form.company} onChange={(e) => upd('company', e.target.value)} placeholder="Ex: JYP Entertainment" />
          </div>
          <div className="form-field">
            <label className="form-label">Année de début <span>*</span></label>
          ...
          <div className="form-field">
            ...
            <input className="input" value={form.fandomName} onChange={(e) => upd('fandomName', e.target.value)} placeholder="Ex: ONCE" />
          </div>
          <div className="form-field">
          ...
          <div className="form-field">
            ...
            <input className="input" value={form.fandomUrl} onChange={(e) => upd('fandomUrl', e.target.value)} placeholder="https://kpop.fandom.com/wiki/..." />
          </div>
        </div>
        ...
        <div className="form-field" style={{ marginTop: 16 }}>
          <label className="form-label">
            ...
            <span style={{ color: 'var(--text-muted)', fontWeight: 400, marginLeft: 6 }}>— uniquement si ce groupe est une sub-unit</span>
          </label>
        ...
        {!isEdit && (
          <div className="form-field" style={{ marginTop: 12 }}>
            ...
            <input className="input" value={form.id} readOnly style={{ opacity: 0.6, fontFamily: 'DM Mono, monospace', fontSize: 13 }} />
          </div>
        )}
      </div>
    </div>
...
// ─── Step 2: Members ────────────────────────────────────────────────────────────
function MembersStep({ members, setMembers, getIdolMatches, idols }: {
...
// ─── Member row editor ─────────────────────────────────────────────────────────
function MemberRowEditor({ member, onUpdate, onRemove, getIdolMatches, idols }: {
  ...
  const selectExisting = (idol: Idol) => {
    onUpdate((m) => ({
      ...m,
      idol: { ...m.idol, id: idol.id, name: idol.name, nationality: idol.nationality, portrait: idol.portrait ?? null, fandomUrl: idol.fandomUrl ?? null },
    ...
    <div className="member-row">
      <div className="member-row__fields">
        <div className="member-row__header">
          <button className="btn btn--danger btn--sm" onClick={onRemove}>🗑 Supprimer</button>
        </div>

        <div className="form-field">
          <label className="form-label">Nom de scène <span>*</span></label>
          <input
            className="input"
        ...
        {hasMatches && (
          <div className="idol-resolution">
            ...
            <div className="idol-matches">
              {matches.map((idol) => (
                <button key={idol.id} className={`idol-match-btn${member.idolResolution.selectedExistingId === idol.id ? ' selected' : ''}`}
                  onClick={() => selectExisting(idol)}>
                  {idol.name} — <span style={{ opacity: 0.6 }}>{idol.primaryGroupId} · {idol.id}</span>
                </button>
              ))}
              <button className="idol-match-btn" style={{ marginTop: 4, color: 'var(--text-muted)' }}
                onClick={() => onUpdate((m) => ({ ...m, idolResolution: { mode: 'new', selectedExistingId: null, resolvedId: null } }))}>
        ...
        {member.idolResolution.mode === 'existing' && (
          <div style={{ fontSize: 12, color: 'var(--accent-teal)', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button className="btn btn--ghost btn--sm" onClick={clearExisting}>Changer</button>
          </div>
        )}
        ...
        <div className="form-grid-2">
          <div className="form-field">
            ...
            <input className="input" value={member.idol.fandomUrl ?? ''} placeholder="https://kpop.fandom.com/wiki/..."
              onChange={(e) => onUpdate((m) => ({ ...m, idol: { ...m.idol, fandomUrl: e.target.value || null } }))} />
          </div>
          <div className="form-field">
            <label className="form-label">Nationalité</label>
            <select className="select" value={member.idol.nationality}
        ...
        <div className="form-field">
          <label className="form-label">Rôles <span>*</span> <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(au moins un)</span></label>
        ...
        <div className="form-field">
          ...
          <input className="input" value={member.idol.portrait ?? ''} placeholder="assets/idols/.../portrait.webp"
...
// ─── Step 3: Songs ──────────────────────────────────────────────────────────────
function SongsStep({ titles, setTitles, bSides, setBSides }: {
  titles: EditableSong[];
...
function SongList({ label, songs, setSongs, showDebutFlag }: {
  label: string;
  songs: EditableSong[];
  ...
  return (
    <div className="songs-section">
      <div className="songs-section__title">
        {label}
        <button className="btn btn--secondary btn--sm" onClick={add}>+ Ajouter</button>
      </div>
      <div className="songs-warning">
        ⚠️ <strong>Vérifiez les noms</strong> — utilisés comme réponses en blind test. Le titre doit être en anglais ou latin, sans caractères coréens.
      </div>
          ...
          <div key={s._key} className="song-row">
            <div className="song-row__fields">
              <div className="song-row__header">
                <button className="btn btn--danger btn--sm" onClick={() => remove(s._key)}>🗑 Supprimer</button>
              </div>
              <div className="form-field">
                <label className="form-label">Titre <span>*</span> <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(nom exact, sans caractères coréens ni crochets)</span></label>
                <input className="input" value={s.title} onChange={(e) => update(s._key, 'title', e.target.value)} placeholder="Ex: Feel Special" />
              </div>
              <div className="form-field">
                <label className="form-label">Lien YouTube <span>*</span></label>
                <input className="input" value={s.youtubeUrl} onChange={(e) => update(s._key, 'youtubeUrl', e.target.value)} placeholder="https://www.youtube.com/watch?v=..." />
              </div>
              <div className="song-row__options">
                <div className="form-field">
                  <label className="form-label">Langue</label>
                  <select className="select" style={{ width: 'auto', minWidth: 120 }} value={s.language}
                ...
                {showDebutFlag && (
                  <label className="checkbox-row">
                    <input type="checkbox" checked={s.isDebutSong}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {s.youtubeUrl && (
                <a href={s.youtubeUrl} target="_blank" rel="noreferrer" className="btn btn--ghost btn--sm" style={{ justifyContent: 'center' }}>
                  ▶ Regarder
                </a>
              )}
            </div>
...
// ─── Step 4: Export ─────────────────────────────────────────────────────────────
function ExportStep({ exportJson, onGenerate, onCopy, onDownload, onBack }: {
  exportJson: string;
  onGenerate: () => void;
  ...
  return (
    <div className="export-section">
      <div className="card export-generate">
        ...
        <button className="btn btn--primary" onClick={onGenerate}>⚡ Générer le JSON</button>
      </div>

      <div className="export-json">
        <div className="export-json__title">JSON du groupe</div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 4 }}>
          <button className="btn btn--secondary btn--sm" onClick={() => onCopy(exportJson)}>📋 Copier</button>
        </div>
        <textarea
      ...
      <div className="export-actions card">
        ...
        <div className="export-actions__buttons">
          ...
          <button className="btn btn--ghost" onClick={onBack}>✏️ Modifier</button>
        </div>
      </div>
    </div>
  );
